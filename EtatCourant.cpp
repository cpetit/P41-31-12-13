...
{
	/* Première méthode (personnelle) finalement non utilisée.
	...
	*/
	...
	// Seconde méthode (de John TROMP) utilisant seulement des shifts
...
bool EtatCourant::jouer(Pion pi,int l,int j)
{
	...
	else
	{
		...
		// On pourrait aussi faire un ou exclusif à la place du + car le pion est ajouté
		// sur une case vide.
...
{
	// l est la colonne où enlever le pion
	// j est le niméro du joueur
	int h,numCase;
	bool ok=false;
	h=this->listeCol[l].getNbPion()-1;
	if (h+1>0)
